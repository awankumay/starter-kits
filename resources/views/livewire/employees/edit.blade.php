...
{{-- modal edit-employee --}}
<flux:modal wire:model.self="showEditEmployeeModal" name="edit-employee" class="md:w-[600px]">
    <div class="space-y-6">
        <div>
            <flux:heading size="lg">Edit Employee</flux:heading>
            <flux:text class="mt-2">Update employee information.</flux:text>
        </div>
        <form wire:submit.prevent="updateEmployee" class="space-y-4">
            @include('livewire.employees.form')
            <div class="flex justify-end space-x-3 pt-4">
                <flux:modal.close>
                    <flux:button type="button" variant="outline" x-on:click="$wire.showEditEmployeeModal = false">Cancel</flux:button>
                </flux:modal.close>
